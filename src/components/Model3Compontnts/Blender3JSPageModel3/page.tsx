...
import Model3textOverlay from "../TextOverlayModel3/page";
...
const animationData = [
  {
    time: 0.0,
    position: [-0.00084, 1.22154, 0.4968],
    quaternion: [0.0, 0.0, 0.0, 1.0],
    fov: 2,
  },
  {
    time: 0.0417,
    position: [-0.08, 1.22, 0.48],
    quaternion: [0.0, -0.35, 0.0, 0.9276399],
    fov: 20,
  },
  ...
  { time: 0.1667, position: [-0.0, 1.216, 0.275], quaternion: [0, 1.0, 0.0, 0.0], fov: 20 },
  // { time: 0.1667, position: [-0.0, 1.216, 0.3], quaternion: [0, 1.0, 0.0, 0.0], fov: 35 },
  { time: 0.2083, position: [-0.0094, 1.2136, 0.0113], quaternion: [0.0, 1.0, -0.00000004, 0.00000004], fov: 30 },
  ...
  { time: 0.3333, position: [-0.0093, 6.6768, 0.0038], quaternion: [-0.0000001, 0.70092404, 0.71323591, 0.0000003], fov: 43.6028 },
  { time: 0.3333, position: [-0.0093, 6.6768, 0.0038], quaternion: [-0.0000001, 0.70092404, 0.71323591, 0.0000003], fov: 43.6028 },
  { time: 0.375, position: [-0.0093, 6.6768, 2.4], quaternion: [-0.50217175, 0.49781877, 0.50217175, 0.49781883], fov: 20.6028 },
...
function HeroTextFade({ scrollProgress }: { scrollProgress: number }) {
  ...
  return (
    ...
    >
      ...
      <Typography variant="hero-body" className="text-[32px] text-[#ABABAB] mt-2">
        The VREC‑H320SC combines real-time driver alerts with built-in G Sensor for emergency recording.
      </Typography>
...
function Blender2JSScene({
  ...
  const dashcamGLTF = useGLTF("/models/VREC_H320SC.glb");
  const { scene: cameraModelScene, nodes: cameraNodes } = useGLTF("/models/VREC_H320SC.glb");
  ...
  if (imagePlaneRef.current) {
    ...
    } else if (scrollProgress >= 0.2042 && scrollProgress <= 0.2376) {
  ...
  useFrame(() => {
    ...
    windshieldObjects.current.forEach((mesh) => {
      ...
      mat.transparent = true;
      mat.opacity = THREE.MathUtils.lerp(0, 1, blend);
      mat.needsUpdate = true;
    });
  });
...
export default function Blender3JSPageModel1() {
  ...
  return (
    <div id="blender2js-scroll-container-model3" ref={containerRef} style={{ height: "2000vh", width: "100%" }}>
      {!modelIsReady && (
      ...
      {modelIsReady && <Model3textOverlay scrollProgress={scrollProgress} />}
