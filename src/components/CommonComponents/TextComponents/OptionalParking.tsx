  ...
  return (
    <>
      <main className={`min-h-screen  text-white relative ${style} text-center sm:text-left`}>
        {/* Top Content Block */}
        <div className="w-full sm:mt-1 mt-[-28rem] text-center sm:max-w-[600px]  max-w-[400px] sm:ml-[6%]">
          {/* Red Label */}
          ...
          {/* Subheading */}
          <p className="text-[#ABABAB]/80 text-[14px] leading-snug ">{subheading}</p>
        </div>

        {/* Disclaimer (Bottom Center on all screens) */}
        <div className=" lg:hidden block h-[110vh] absolute inset-0 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none " />

        {/* <p className="absolute bottom-6 sm:max-w-[600px]  max-w-[400px] sm:bottom-10 ms-6 text-[13px] text-[#4B4B4B] text-center px-4">
          {description}
        </p> */}
         <Typography variant="section-card-body" className="absolute bottom-6 sm:max-w-[600px]  max-w-[400px] sm:bottom-10 ms-6 text-[13px] text-[#4B4B4B] text-center px-4">{description}</Typography>
      </main>
    </>
  );
}
